...
The intelligent controller layer is physically implemented in Python utilizing stable Proximal Policy Optimization (PPO). The custom \texttt{ORANns3Env} gym-wrapper provides a strictly verified OpenAI environment interface, absorbing the raw KPMs and outputting RL float actions. State representation consists of 48 deep physical features per cell, which are frame-stacked across three historical intervals, explicitly producing exactly 144 feature inputs per cell. This feeds into the core transformer architecture. The action space is rigidly bounded by real-world physics, controlling continuous normalized output bounds for Transmit Power ($[10, 46]$ dBm), CIO ($[-6, 6]$ dB), and TTT ($[0, 1280]$ ms).
