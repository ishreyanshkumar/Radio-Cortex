...
E2InterfaceManager::SetupKafka()
{
    ...
    rd_kafka_topic_partition_list_add(topics, "e2_rc_control", RD_KAFKA_PARTITION_UA);
...
E2InterfaceManager::CollectCellMetrics()
{
    ...
    for (uint32_t i = 0; i < m_enbNodes.GetN(); ++i)
    {
        ...
        cellMetric.queueLength = 0; // Hard to get aggregate queue without iterating all UEs/Bearers
...
main(int argc, char* argv[])
{
    ...
    if (enableE2)
    {
        e2Manager = Create<E2InterfaceManager>(lteHelper, enbNodes, ueNodes, kafkaBrokers);
