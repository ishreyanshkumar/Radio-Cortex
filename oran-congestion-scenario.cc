...
class MetricCollector : public SimpleRefCount<MetricCollector>
{
  ...
  private:
    ...
    std::map<uint32_t, uint32_t> m_lastLost;             // Key: UE index, value: cumulative lost
...
void
MetricCollector::UpdateRntiMapping(uint32_t ueIndex, uint16_t rnti)
{
    if (rnti == 0)
    {
        return;
...
void
MetricCollector::RegisterUeTraces(NodeContainer ues)
{
            ...
            if (server)
            {
                ...
                if (lteDev)
                {
                    ...
                    server->TraceConnectWithoutContext(
                        "Rx",
                        MakeCallback(&MetricCollector::ReportAppRx, this, i));
...
void
MetricCollector::RegisterEnbTraces(NodeContainer enbs)
{
    for (uint32_t i = 0; i < enbs.GetN(); ++i)
    {
        ...
        {
            Ptr<LteEnbNetDevice> lteDev = enb->GetDevice(j)->GetObject<LteEnbNetDevice>();
            if (lteDev)
            {
...
void
MetricCollector::ReportUeSinr(uint16_t cellId,
                              uint16_t rnti,
                              double rsrp,
                              double sinr,
                              uint8_t componentCarrierId)
...
void
MetricCollector::ReportDlScheduling(DlSchedulingCallbackInfo info)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
void
MetricCollector::ReportAppRx(uint32_t ueIndex, Ptr<const Packet> packet)
{
    SeqTsHeader seqTs;
...
E2InterfaceManager::EnableE2()
{
    ...
    m_pollEvent = Simulator::Schedule(MilliSeconds(1), &E2InterfaceManager::PollKafka, this);
}
...
E2InterfaceManager::SetupKafka()
{
    ...
    if (rd_kafka_conf_set(conf, "bootstrap.servers", m_brokers.c_str(), errstr, sizeof(errstr)) !=
        RD_KAFKA_CONF_OK)
    {
    ...
    rd_kafka_topic_partition_list_add(topics, rcTopicName.c_str(), RD_KAFKA_PARTITION_UA);

    rd_kafka_resp_err_t err = rd_kafka_subscribe(m_consumer, topics);
    if (err)
    {
        NS_LOG_ERROR("Failed to subscribe to e2_rc_control: " << rd_kafka_err2str(err));
    }
...
E2InterfaceManager::PollKafka()
{
    ...
    m_pollEvent = Simulator::Schedule(MilliSeconds(10), &E2InterfaceManager::PollKafka, this);
}

std::map<uint32_t, E2InterfaceManager::UeMetrics>
E2InterfaceManager::CollectUeMetrics(const std::map<uint32_t, UeMetricAccumulator>& realMetrics)
{
...
E2InterfaceManager::CollectCellMetrics(const std::map<uint32_t, UeMetricAccumulator>& realMetrics)
{
    ...
    for (uint32_t ueIdx = 0; ueIdx < m_ueNodes.GetN(); ++ueIdx)
    {
        if (realMetrics.count(ueIdx))
        {
            ...
            if (cellId >= 0 && cellId < (int32_t)numCells)
            {
                ...
                queuePerCell[cellId] += static_cast<double>(acc.bytesRx > 0 ? acc.packetsLost : 0);
            }
        }
    }
    ...
    for (uint32_t i = 0; i < numCells; ++i)
    {
        ...
        cellMetric.rbUtilization =
            (totalRbsPerInterval > 0)
                ? std::min(1.0, static_cast<double>(rbsPerCell[i]) / totalRbsPerInterval)
                : 0.0;
...
E2InterfaceManager::SendKpmReport()
{
    ...
    // Collect metrics (Fetch ONCE to ensure consistency between UE and Cell metrics)
    ...
    // Send via Kafka
    if (rd_kafka_produce(m_kpmTopic,
                         RD_KAFKA_PARTITION_UA,
                         RD_KAFKA_MSG_F_COPY,
                         (void*)kpmString.c_str(),
                         kpmString.length(),
                         NULL,
                         0,
                         NULL) == -1)
        ...
        if (cellPos != std::string::npos)
        {
            ...
            if (parseValue("\"hysteresis_db\":", hysteresis))
            {
                ...
                if (rrc)
                {
                    ...
                    if (hoAlgo)
                    {
                        ...
                        NS_LOG_INFO("Set Cell " << i << " Hysteresis to " << hysteresis << " dB");
                    }
                }
            }
            ...
            if (parseValue("\"max_harq_tx\":", maxHarq))
            {
                ...
                if (mac)
                {
                    ...
                    NS_LOG_INFO("Set Cell " << i << " MaxHarq to " << static_cast<int>(maxHarq));
                }
            }
            ...
            {
                NS_LOG_INFO("Cell " << i << " SchedulerWeight = " << schedWeight << " (logged)");
            }
        }
    }
}
    ...
    try
    {
        ...
        else
        {
            ...
            ueMobility.SetPositionAllocator(
                ...
                "X",
                StringValue(
                    "ns3::UniformRandomVariable[Min=0|Max=" + std::to_string(numCells * 500) + "]"),
                "Y",
        ...
        ApplicationContainer trafficApps = SetupTraffic(ueNodes, remoteHostContainer, lteHelper);
        ...
        {
            e2Manager =
                Create<E2InterfaceManager>(lteHelper, enbNodes, ueNodes, kafkaBrokers, topicSuffix);
        ...
        {
            // Start congestion much earlier (0.2s) to see effects in short RL episodes
            scenarioManager->ActivateScenario(CongestionScenarioManager::FLASH_CROWD, Seconds(0.2));
        }
        ...
        {
            scenarioManager->ActivateScenario(CongestionScenarioManager::TRAFFIC_BURST,
                                              Seconds(0.2));
        }
        ...
        {
            scenarioManager->ActivateScenario(CongestionScenarioManager::SLEEPY_CAMPUS,
                                              Seconds(0.2));
        }
        ...
        {
            scenarioManager->ActivateScenario(CongestionScenarioManager::COMMUTER_RUSH,
                                              Seconds(0.2));
        }
        else if (congestionScenario == "mixed_reality")
        {
            scenarioManager->ActivateScenario(CongestionScenarioManager::MIXED_REALITY,
                                              Seconds(1.0));
        }
        ...
        {
            scenarioManager->ActivateScenario(CongestionScenarioManager::SPECTRUM_CRUNCH,
                                              Seconds(0.5));
        }

        NS_LOG_INFO("Starting simulation...");
        // Ipv4GlobalRoutingHelper::PopulateRoutingTables(); // Caused crash with LTE
        // Optimize performance by disabling verbose logging
